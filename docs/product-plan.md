...
12. Housekeeping to settle before the first package version, since a custom metadata type is effectively permanent once packaged. `Naming_Pattern__mdt` (four fields, six shipped records) has no consumer anywhere; naming reads the three pattern fields on Nonprofit Settings instead. Three permission set groups, Fundraising Staff, Program Staff and Volunteer Coordinator, each contain only Nonprofit Staff. Both were excluded from the 2026-09-15 pass at Brandon's instruction to leave custom metadata alone. Decision needed before the first package version, not after.
