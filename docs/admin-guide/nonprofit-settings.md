...
2. Click the **Nonprofit Settings** tab. The console opens with the section list on the
   ...
   Households, Relationships, Addresses, Giving, Receipts, Rollups, Automation, Access,
   Import, and Health. You only see the ones that have something in them, so a fresh install with no
   modules shows fewer.
