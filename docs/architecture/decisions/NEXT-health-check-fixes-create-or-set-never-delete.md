...
rows and import templates are created from custom metadata by the install script or when a page
first opens, so an org can be missing rows it is meant to have. And the nightly rollup run is
...
2. **Package owned configuration only.** A fix writes rows materialized from this package's own
   ...
   constituent, a gift, an address or an amount.
...
5. **It says what it will do, counted from this org, and asks first.** The finding names the
   rows or the people the fix touches. The panel shows that text again with a Confirm button
   before any fix that changes something runs. Afterwards the report is re-run underneath it.
...
Every fix is reversible from the app: a created definition or template can be switched off or
deleted on its own page, and the nightly run stopped on the Rollups page. Health Check itself
...
- A restored automation switch takes the registry's `Enabled_By_Default__c`. Every shipped
  ...
  running (a missing switch means it runs). A registry row shipped off by default would break
  condition 3 for this fix, and is re-examined before it ships.
...
- A new fix is added by passing the six conditions in review, one at a time.
