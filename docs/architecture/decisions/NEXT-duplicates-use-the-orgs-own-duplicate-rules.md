...
- **A people merge keeps what it touches.** The platform's merge deletes the merged-away
  ...
  real-time totals that target Contact are recalculated at once with the Rollups page's full
  recalculation, queued once per transaction (`RollupService.runAfterMerge`, driven by the
  definitions that target Contact, so Core names no Giving object, ADR-0014). The engine's own
  after-delete merge handling queued nothing in a test run, and its recalculation limited to
  named parents fails inside the vendored processor, so the whole recalculation is used. It
  costs one full recalculation of those definitions per merge transaction, which is what the
  nightly run does anyway; revisit if bulk merging makes that too heavy. An emptied household is left for a person to merge into the
