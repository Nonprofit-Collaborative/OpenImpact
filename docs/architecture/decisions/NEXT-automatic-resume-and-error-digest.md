...
1. **Lazy evaluation stays the source of truth; a one-time job records the end.** Pausing writes
   ...
   schedule. The job carries the end it was scheduled for and, holding a lock on the settings
   record, clears the pause only while the stored end is still that one and has passed: a job
   that outlived a new pause changes nothing, and a job the platform fires early (a clock
   change can do it) schedules itself again for the end instead of removing itself. Resuming
   by hand, or pausing again, cancels the job first. If the job cannot be scheduled, the pause
   still happens and still ends on time, and the failure goes to the Error Log.
...
4. **The digest counts and links; it never quotes.** It carries the number of entries created
   ...
   Record Reference. Every count is capped: the run reads at most the newest 2,000 entries,
   says "more than 2,000" beyond that, and first checks how many query rows the transaction
   has left, so a flooded Error Log still gets its digest instead of stopping it for good.
...
7. **A digest that stopped running is a Health Check warning.** When the digest is scheduled
   but its last run is more than two days old (the job's owner was deactivated, for example),
   Health Check says so.
8. **System mode writes to the settings go through one writer.** `OrgSettingsSystemWriter`
   locks the stored record and updates only the keys it is given, or, on an org that has never
   saved its settings, creates the record from the shipped defaults (ADR-0035) and puts the
   blanks back. The pause, the seasonal swap's run record and the digest's run record all save
   through it, so none of them can switch a shipped feature off or confirm a mode nobody chose.
...
- The resume is recorded as changed by the administrator who paused, because the job runs as
  that person. It is stamped when the job ran, which is at or a moment after the pause's own
  end; a resume by hand is always earlier than the end it cut short. That is how the change
  list tells an automatic resume from a manual one.
