...
- **Receipt to its stored file**, one to one, as a `ContentVersion` linked to the receipt
  ...
  role can read a Contact and an Account (R-RC11, ADR-NEXT).
...
check `Manage_Nonprofit_Settings` and write in user mode (ADR-NEXT).
