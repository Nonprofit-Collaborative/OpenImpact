...
| v0.5 | 2026-09-08 | C-21 Health Check v2. No object, field or rollup row added. Health Check gains fix actions, whose admissibility rule is ADR-0029, and four checks that read state the model already defines: packaged rollup definitions, automation switch rows and import templates that were never materialized from their shipped defaults (Section 13, rules R-R6, R-A2, R-IT4), and people who hold the `Nonprofit_Admin` permission set without the role that carries it. Recorded against R-A2: an `Automation_Setting__c` row whose `Automation_Registry__mdt` entry is no longer shipped is reported and left alone, never removed, because no fix action deletes a record (ADR-0029 condition 3); nothing runs an automation with no registry row behind it, so the row is inert. |
